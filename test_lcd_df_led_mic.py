...
def play_alarm():
    ...
    lcd_show("ALARM!", f"{ALARM_HOUR:02d}:{ALARM_MINUTE:02d}")
...
def vosk_callback(indata, frames, time_info, status):
    ...
    if rec.AcceptWaveform(resampled_bytes):
        ...
        if recognised and recognised != "[unk]":
            hour, minute = parse_time(recognised)
            if hour is not None:
                ALARM_HOUR   = hour
                ALARM_MINUTE = minute
                msg = f"{hour:02d}:{minute:02d}"
                print(f"Alarm set for {msg}")
                lcd_show("Alarm set!", msg)
            else:
                print("Didn't understand the time, try again...")
                lcd_show("Didn't understand", "Try again...")
        else:
...
def update_lcd_idle():
    """Continuously update LCD with date/day when idle."""
    while True:
        if not listening:
            now = datetime.now()
            line1 = now.strftime('%a %d %b %Y')
            # Show alarm time on line 2 if set
            if ALARM_HOUR is not None:
                line2 = f"Alarm {ALARM_HOUR:02d}:{ALARM_MINUTE:02d}"
            else:
                line2 = "No alarm set"
...
def main():
    ...
    # Start LCD idle update thread
    threading.Thread(target=update_lcd_idle, daemon=True).start()
